...
class ListCommands extends Command {
	...
	public async runCommand(message: Message, args: MessageCommandArgument[]): Promise<void> {
		const filteredCommands = Object.values(clientCommands).filter(a=>`${a.name}\x00${a.aliases.join("\x00")}`.includes((args[0] || "").toString()))
		const commandList = filteredCommands.map(cmd => `-# \`${cmd.syntax}\``).join("\n");
		await message.reply({
			content: filteredCommands.length === 0 ? "no commands :(" : `**syntax:** command <required> [optional]\n${commandList}`
		});
	}
}
