...
import { agent, assertAtprotoCredentials, getModPolicy, LABELER_DIDS } from "./util.m";
...
class HelloCommand extends Command {
	...
	public async runCommand(message: Message, args: MessageCommandArgument[]): Promise<void> {
		...
		const fileContent = `**${profile.displayName || profile.handle}** ${profile.did}
**${profile.followersCount}** followers
**${profile.followsCount}** following

${profile.description}

${profile.viewer?.blocking ? "You are blocking this user\n" : ""}${profile.viewer?.blockedBy ? "You are being blocked by this user\n" : ""}
${(profile.labels || []).map((a) => {
	if (a.val === "!no-unauthenticated") return `${profile.displayName || profile.handle}: Logged-in users only - This is a global label value defined by the AT Protocol (!no-unauthenticated)`
	const p = getModPolicy(`${a.src}/${a.val}`);
	if (!p) return `Invalid Label ${a.src}/${a.val}`;
	return `${p.labeler}: ${p.name} - ${p.description} (${p.id})`}).join("\n")
}`
		await message.reply({
			files: [
				{
